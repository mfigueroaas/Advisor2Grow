...
export default function BrandCard({ brand, isActive, onActivate }: BrandCardProps) {
  return (
    <button
      ...
      className={cn(
        "relative h-32 md:h-48 flex flex-col items-center justify-center transition-all duration-300 group cursor-pointer rounded-xl overflow-hidden bg-surface border border-border hover:border-brand hover:shadow-lg",
      ...
      <span
        className={cn(
          ...
          isActive
            ...
            : "text-faint-foreground group-hover:text-brand"
        )}
      >
        {brand.name.split(" ")[0]}
      </span>
